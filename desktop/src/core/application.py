    ...
    def ensure_single_instance(self):
        if PLATFORM == WINDOWS:
            ...
            if ctypes.windll.kernel32.GetLastError() == 183:  # ERROR_ALREADY_EXISTS
                CustomDialog(
                    "Another instance of SkyClip is already running.",
                    msg_type="warning",
                ).mainloop()
                sys.exit(0)
        elif PLATFORM == MACOS or PLATFORM.startswith(LINUX):
            ...
            try:
                ...
                run_anyway = MessageBox().askquestion(
                    "SkyClip",
                    "Another instance of SkyClip is already running. Do you want to run anyway?",
                )
                if run_anyway == "yes":
            ...
            if login_successful:
                ...
                if ws_conn_successful:
                    ...
                    if display_login_success_dialog:
                        CustomDialog(
                            "Success! SkyClip will now run in the task bar/menu bar.",
                            msg_type="success",
                            timeout=5000,
                        ).mainloop()
                    break

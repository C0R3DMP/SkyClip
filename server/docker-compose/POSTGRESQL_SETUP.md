...
- Automatically create the database schema on first startup (`schema.sql`,
  run via Spring's `spring.sql.init.mode=always` — SkyClip does not use Flyway)

### 3. Verify the Connection

```bash
docker logs clipcascade | grep -i "started SkyClipApplication\|HikariPool-1 - Start completed"
```

You should see the Hikari connection pool start and the application come up cleanly.
...
- The schema is created automatically from `schema.sql` on first startup
...
1. Export data from H2:
   ...
   # H2's CIPHER=AES password is "<file password> <user password>" — a single
   # value containing a space, so it MUST be quoted or the shell splits it into
   # two separate (wrong) arguments and authentication fails.
...
### Rate-Limiting (`login_attempts`) Schema

The `login_attempts` table (used for API rate limiting) is defined once in
`schema.sql` using `GENERATED BY DEFAULT AS IDENTITY`, which both H2 and
PostgreSQL accept, so the same script creates a compatible table on either
database — no separate migration step is needed.
...
### Schema creation fails on startup

**Cause:** `schema.sql` failed to apply — usually a stale/incompatible volume
from an older SkyClip version, or the database user lacking permission to
create tables.

**Solution:**
1. Check the SkyClip startup logs for the SQL error: `docker logs clipcascade`
2. If the volume is stale and can be discarded: `docker-compose down && docker volume rm postgres-data && docker-compose up`
...
- Test login rate limiting on PostgreSQL

...
@Controller
public class SkyClipController {
    ...
    public String login(Model model) {
        model.addAttribute("signupEnabled", appProperties.isSignupEnabled());
        model.addAttribute("donationsEnabled", appProperties.isDonationsEnabled());
        return "login"; // login.html
    }

    @GetMapping("/signup")
    public String signup() {
        if (appProperties.isSignupEnabled()) {
    ...
    public String donate() {
        if (appProperties.isDonationsEnabled()) {
    ...
    public String advance(
        ...
        if (userPrincipal.isAdmin()) {
            model.addAttribute("envVariables", appProperties);
    ...
    public ResponseEntity<?> getStunUrl() {
        return ResponseEntityUtil.executeWithResponse(() -> {
            if (appProperties.isP2pEnabled()) {
                return Collections.singletonMap("url", appProperties.getP2pStunUrl());
            } else {
    ...
    public ResponseEntity<?> getMaxSizeAllowed() {
        return ResponseEntityUtil.executeWithResponse(
                () -> Collections.singletonMap("maxsize",
                        appProperties.getMaxMessageSizeInBytes()));
    }
    ...
    public ResponseEntity<?> getMaxMessageSizeLimit() {
        return ResponseEntityUtil.executeWithResponse(
                () -> Collections.singletonMap("maxmessagesize",
                        appProperties.getMaxMessageSizeInMiB()));
    }
